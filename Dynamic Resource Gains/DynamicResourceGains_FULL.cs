...
namespace Dynamic_Resource_Gains
{
    ...
    public class DynamicResourceGains : MonoBehaviour
    {
        ...
        private void OnAppLauncherButtonToggleOn() { guiVisible = true; }
        private void OnAppLauncherButtonToggleOff() { guiVisible = false; }

        private void OnGUI()
        {
            if (!guiVisible || HighLogic.LoadedScene != GameScenes.SPACECENTER) return;

            if (windowStyle == null) InitializeStyles();

            windowRect = GUILayout.Window(GetInstanceID(), windowRect, DrawWindow,
                "Dynamic Resource Gains", windowStyle,
                GUILayout.MinWidth(350), GUILayout.MinHeight(200));
        }
        ...
        private void DrawWindow(int windowID)
        {
            ...
            if (dynamicScience) DrawScienceSettings();

            dynamicFunds = GUILayout.Toggle(dynamicFunds, "Enable Dynamic Funds");
            if (dynamicFunds) DrawFundsSettings();

            dynamicReputation = GUILayout.Toggle(dynamicReputation, "Enable Dynamic Reputation");
            if (dynamicReputation) DrawReputationSettings();

            GUILayout.Space(10);
            GUILayout.BeginHorizontal();
            if (GUILayout.Button("Apply", buttonStyle)) ApplyMultipliers();
            if (GUILayout.Button("Save", buttonStyle)) SaveConfiguration();
            GUILayout.EndHorizontal();
        ...
        private void ApplyMultipliers()
        {
            ...
            if (HighLogic.CurrentGame != null && HighLogic.CurrentGame.Parameters != null)
            {
                ...
                careerParams.ScienceGainMultiplier = CalculateScienceMultiplier();
                careerParams.FundsGainMultiplier = CalculateFundsMultiplier();
                careerParams.RepGainMultiplier = CalculateReputationMultiplier();
                Debug.Log($"[DRG] Applied multipliers - Science: {careerParams.ScienceGainMultiplier}, Funds: {careerParams.FundsGainMultiplier}, Rep: {careerParams.RepGainMultiplier}");
            }
        }

        private float CalculateScienceMultiplier()
        {
            if (!dynamicScience) return baseScienceMultiplier;
        ...
        {
            if (!dynamicFunds) return baseFundsMultiplier;
        ...
        {
            if (!dynamicReputation) return baseRepMultiplier;
        ...
        {
            if (dynamicScience || dynamicFunds || dynamicReputation) ApplyMultipliers();
        }
